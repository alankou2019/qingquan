...
class PointStoresController extends FrontendBaseController
{
	/**
	 *
	 * desc 归档列表
	...
	public function listAction()
	{
		...
		if($isAjax)
		{
			...
			$this->view->render('pointstores','list');
			$this->view->finish();
	...
	{
		$backurl = $this->getHelper()->createUrl(array('p'=>'pointstores/list'));  //默认跳回到人员列表
	...
	public function exportAction()
	{
		...
		$data   = $helper->exportCsv($data,$total_page,$new_name,3);
	...
	public function exportDetailAction()
	{
		...
		$data=PointStoresReportItemDetailModel::getExportData($id,$sid);
		if (!$data)
		{
		...
		$new_name = '积分考评表详细评分';
		$helper = Helper::factory();
		$data   = $helper->exportCsv($data,$total_page,$new_name,4);
	...
	protected  function _getReportItemDataList()
	{
		...
		$columns='i.id,i.quota_id,i.quota_value,i.report_user_id,q.type,'.
		...
		$items = $this->modelsManager->createBuilder()
									->columns($columns)
									->addFrom('ScshuxCms\Dacang\Model\PointStoresReportItemModel','i')
	...
	protected  function _getDataList()
	{
		...
		$countInfo = PointReportModel::factory()->getModelsManager()->createBuilder()
											...
											->leftJoin('ScshuxCms\Dacang\Model\PointStoresReportItemModel','r.id=s.report_id','s')
		...
		$columns.= 's.id as sid,s.quota_value';
		...
		if($items)
		{
			...
			{
				$item['totalpoint'] = PointStoresReportItemModel::factory()->getTotalPoint($item['id'],$item['sid']);
			}
		}	
	...
	protected function _getShowPointList()
	{
		...
		$pointstoresitem=PointStoresReportItemModel::findFirst($sid);
		$storestime=$pointstoresitem->storestime;
		/*条件*/
		$page = isset($_REQUEST['page'])?intval($_REQUEST['page']):1;
		$page = $page<1?1:$page;
		$pagesize = isset($_REQUEST['pagesize'])?intval($_REQUEST['pagesize']):15;
		...
		//获取数据列表
		$dataList = PointStoresReportItemDetailModel::showPoint($reportId,$quotaId,$storestime,$page,$pagesize);
	...
	protected  function  _remove($ids)
	{
		if($ids){
			...
			{
				$storesReport=PointStoresReportItemModel::findFirst($id);
				if (!$storesReport)
				{
					continue;
				}
				...
				$where='report_id='.$reportId.' and storestime='.$storestime;
				PointStoresReportItemModel::factory()->deleteBySql($where);	//删除归档具体指标
				PointStoresReportItemDetailModel::factory()->deleteBySql($where);	//删除归档具体指标的 考评记录
			}
		}
	}
	/**
