<div class="warp salary_mobile_page" id="evaluation">
	<style>
		...
		.salary_mobile_page .quick_row{display:flex;border-top:1px solid #edf0f5;}
		.salary_mobile_page .quick_item{flex:1;padding:0.78rem 0.35rem;text-align:center;color:#111827;}
		.salary_mobile_page .quick_item:first-child{border-right:1px solid #edf0f5;}
		.salary_mobile_page .quick_item .num{font-size:0.86rem;font-weight:bold;color:#1f8fd8;line-height:1.05rem;}
		.salary_mobile_page .quick_item .txt{font-size:0.55rem;color:#64748b;line-height:0.8rem;margin-top:0.12rem;}
	...
	<div class="salary_hero">
		...
		<div class="title">我的工资条</div>
		<div class="sub">仅展示已发放工资条，未发放月份不会提前显示金额。</div>
	</div>
	<div class="salary_block">
		<div class="salary_block_title">当月工资条</div>
		{% if monthSlip %}
		<div class="month_card">
			...
			<a class="salary_btn" href="{{helper.createUrl(['p':'bs/salarydetail','id':monthSlip['id']])}}">查看工资条</a>
		</div>
		{% else %}
		<div class="empty">{{currentMonth}} 暂无已发放工资条</div>
		{% endif %}
		<div class="quick_row">
			<a class="quick_item" href="{{helper.createUrl(['p':'bs/salaryyear'])}}">
				<div class="num">{{yearSlipCount}}</div>
				<div class="txt">当年薪酬</div>
			</a>
			<a class="quick_item" href="{{helper.createUrl(['p':'bs/salaryhistory'])}}">
