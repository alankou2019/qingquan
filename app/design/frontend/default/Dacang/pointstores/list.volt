...
<script src="/skin/adminhtml/default/libs/laydate/laydate.js"></script>
    ...
    <div class="search_box">
        ...
        <input type="text" placeholder="开始日期" class="search laydate-icon dateinput"  name="start_time" >
        <input type="text" placeholder="结束日期" class="search laydate-icon dateinput"  name="end_time" >
        <!--搜索输入框-->
        <!--搜索按钮-->
        <button class="search_btn btn1"  type="button" onclick="searchData();"><i class="iconfont icon-llhomesearch"></i>搜索</button>
        </form>
        <!--搜索按钮-->
        <button class="operate" onclick="exportDatat(1,this)" data-key="{{unique_key}}" data-url="{{helper.createUrl(['p':'pointstores/export'])}}"><i class="iconfont icon-daochuexcel"></i>导出</button>
        <!--右侧配置-->
...
{% endif %}
    <table class="table_box">
        ...
        <tr class="table_head">
            ...
            <th class="w100"><span>被评分人姓名</span></th>  
            <th class="w250"><span>所属部门</span></th>  
            <th class="w200"><span>考核表名称</span></th>  
            <th class="w200"><span>总分数</span></th>  
        ...
        {% for item in dataList.items %}
        	<tr>
            	<td class="check_box"><label class="radio_check"><input type="checkbox" name="radio_check" class="item_checkbox" value="{{item['sid']}}"/></label></td>
                <td class="operate">
                <div class="handle">
					...
					<ul class="list">
						<li><a href="{{helper.createUrl(['p':'pointstores/detail','id':item['id'],'sid':item['sid']])}}">查看详情</a></li>
						<li>
                            <a href="javascript:void(0);"  onclick="exportDatat(1,this)" data-key="{{unique_key}}{{item.id}}" 
                            data-url="{{helper.createUrl(['p':'pointstores/exportdetail','id':item['id'],'sid':item['sid']])}}">导出详情</a>
                        </li>
					</ul>
				</div>
                    <button class="delete" onclick="listTable.remove({{item['sid']}},'您确认要删除吗?');"><i class="iconfont icon-shanchu"></i>删除</button>
                </td>
