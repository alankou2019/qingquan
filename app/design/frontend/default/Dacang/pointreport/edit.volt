...
<div class="full_box">
	<div class="full_title">
		...
		<a class="go_back" onclick="window.location='{{helper.createUrl(['p':'pointreport/list'])}}';">
	...
	<div class="full_cont">
		<!--表单-->
		<form action="{{helper.createUrl(['p':'pointreport/save'])}}" method="post" class="form_full" id="dataForm" name="dataForm" enctype="multipart/form-data">
			<!--报表id-->
			...
			<ul class="list_form_full">
				...
				{% if  quotaapplynum>0 %}
				<!--分割线-->
                <div class="sub_title quotaApplydiv"></div>
                <li class="posi_lm quotaApplyli">
                    <div class="left posi_l must">待审核指标:</div>
                    <div class="right posi_m reportitem">
                        <table border="1"  width="60%" class="table_box">
                            <tr>
                                <th>指标名称</th>
                            ...
                            <tbody >
                                {% for applyquota in quotaapply %}
                                <tr class="quotaApplytr">
                                    <td>
                                        <span>{{helper.substr(applyquota.name,0,15)}}</span>
                                    </td>
                                    <td>
                                        <span>{{helper.substr(applyquota.point_desc,0,30)}}</span>
                                    </td>
                                    <td>
                                        {{quotatype[applyquota.type]}}
                                    </td>
                                    <td class='pingfenren'>
                                        <span>{{applyquota.username}}</span>
                                    </td>
                                    <td >
                                        <button type="button" onclick="doUse(this)"  applyid="{{applyquota.id}}"  option="yes" class="mybutton"> 启用  </button>
                                        <button type='button' onclick='doUse(this)'  applyid="{{applyquota.id}}"  option="no" class="mybutton"> 不启用 </button>
                                    </td>
			...
			<div class="online_btn_box">
				<button type="reset" class="f_btn" onclick="window.location='{{helper.createUrl(['p':'pointreport/list'])}}';" >返回</button>
...
<script>
	...
	function showpoint(reportId,quotaId)
	{
		...
		layer.open({
		    ...
		    shadeClose: true,
		  	shade: 0.8,
		  	area:['600px','520px'],
		  	content:"{{helper.createUrl(['p':'pointreport/showpoint'])}}"+'?reportId='+reportId+'&quotaId='+quotaId,
		  	btn: ['确定', '取消'],
		  	yes: function(index){
		  		layer.close(index);
		  	}
	...
	function changequota()
	{
		layer.open({
		    ...
		    shadeClose: true,
		  	shade: 0.8,
		  	area: ['60%', '80%'],
		  	content:"{{helper.createUrl(['p':'quota/simplelist','type':'point'])}}",
		  	btn: ['确定', '取消'],
		  	yes: function(index){
	...
	function afterquota(res)
	{
		...
		for(var i=0;i<len;i++){
			...
			html += "<input type='hidden' name='reportuserid[]' value='' class='reportuserids'/></td>";
	...
	function submitform(obj)
	{
	    ...
	    $.ajax({
	   		 type: "POST",
	   		 url: "{{helper.createUrl(['p':'pointreport/save'])}}",
	   		 data: $('#dataForm').serialize(),
	   		 dataType: "json",
	   		 success: function(res){
			 	if(res.status == 'y'){
			 		layer.msg('考评表设置成功',function(index){
			 			window.location.href = "{{helper.createUrl(['p':'pointreport/list'])}}";
			 		}) ;
	...
	{
		var obj=$(obj);
		var id=obj.attr('applyid');
		var option=obj.attr('option');
		
		$.ajax({
            type: "POST",
            url: "{{helper.createUrl(['p':'pointreport/setquota'])}}",
            data: {'quotaId':id,'option':option},
            dataType: "json",
            success: function(res){
               if(res.status == 'y'){
                   obj.parent().parent().remove();
	...
	function doUseAfter()
	{
		var quotaapplylen=$('.quotaApplytr').length;
		if(quotaapplylen==0){
			$('.quotaApplyli').remove();
			$('.quotaApplydiv').remove();
		}
	}
