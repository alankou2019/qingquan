...
<script src="/skin/adminhtml/default/libs/layer/layer.js" type="text/javascript" charset="utf-8"></script>
    ...
    <div class="head_tab clear">
        <ul>
            <li class="on"><a href="#">积分考评表列表</a></li>
        </ul>
    </div>
    <!--头部TAB切换-->
        ...
        {% for item in dataList.items %}
        	...
        	<tr>
                ...
                <div class="handle">
					...
					<ul class="list">
						<li><a href="{{helper.createUrl(['p':'pointreport/detail','id':item['id']])}}">查看详情</a></li>
							<!--进行考评  既是发消息给评分人  提示进行考核-->
							{% if item['ispoint'] == 0 %}
								<li><a href="{{helper.createUrl(['p':'pointreport/edit','id':item['id']])}}">编辑</a></li>
	 ...
	 function savestores(id)
	 {
		 ...
		 layer.confirm('你确定要进行归档操作么？', {
			  btn: ['确定','取消'] //按钮
				...
				$.ajax({
			   		 type: "POST",
			   		 url: "{{helper.createUrl(['p':'pointreport/stores'])}}",
			   		 data: {id:id},
			   		 dataType: "json",
     ...
     function allsavestores()
     {
         ...
         layer.confirm('你确定要进行所有的归档操作么？', {
              btn: ['确定','取消'] //按钮
                ...
                $.ajax({
                     type: "POST",
                     url: "{{helper.createUrl(['p':'pointreport/allstores'])}}",
                     dataType: "json",
                     success: function(res){
                        var res = eval(res);
                         if(res.status == 'y'){
	 ...
	 function pointing(id)
	 {
		 ...
		 layer.confirm('进行考评后将不能在编辑，你确定要进行考评操作么？', {
			  btn: ['确定','取消'] //按钮
				...
				$.ajax({
			   		 type: "POST",
			   		 url: "{{helper.createUrl(['p':'pointreport/pointing'])}}",
			   		 data: {id:id},
			   		 dataType: "json",
			   		 success: function(res){
					 	var res = eval(res);
     ...
     function allpointing()
     {
         layer.confirm('你确定要进行所有的考评操作么？', {
              btn: ['确定','取消'] //按钮
                ...
                $.ajax({
                     type: "POST",
                     url: "{{helper.createUrl(['p':'pointreport/allpointing'])}}",
                     dataType: "json",
                     success: function(res){
                        var res = eval(res);
                         if(res.status == 'y'){
     ...
     function comeback(id)
     {
         ...
         layer.confirm('只能撤销正在进行中的考评，你确定操作么？', {
              btn: ['确定','取消'] //按钮
                ...
                $.ajax({
                     type: "POST",
                     url: "{{helper.createUrl(['p':'pointreport/comeback'])}}",
                     data: {id:id},
                     dataType: "json",
                     success: function(res){
                        var res = eval(res);
     ...
     function resetPoint(id)
     {
         ...
         layer.confirm('重置后将清空已提交的自评和评分，评分人可重新提交，确认重置么？', {
              btn: ['确定','取消'] //按钮
                ...
                $.ajax({
                     type: "POST",
                     url: "{{helper.createUrl(['p':'pointreport/resetpoint'])}}",
                     data: {id:id},
                     dataType: "json",
                     success: function(res){
                        var res = eval(res);
	 ...
	 function pointed(id)
	 {
		 ...
		 layer.confirm('你确定要进行公示操作么？', {
			  btn: ['确定','取消'] //按钮
				...
				$.ajax({
			   		 type: "POST",
			   		 url: "{{helper.createUrl(['p':'pointreport/pointed'])}}",
			   		 data: {id:id},
			   		 dataType: "json",
			   		 success: function(res){
					 	var res = eval(res);
     ...
     function allpointed()
     {
         layer.confirm('你确定要进行所有的公示操作么？', {
              btn: ['确定','取消'] //按钮
                ...
                $.ajax({
                     type: "POST",
                     url: "{{helper.createUrl(['p':'pointreport/allpointed'])}}",
                     dataType: "json",
                     success: function(res){
                        var res = eval(res);
                         if(res.status == 'y'){
