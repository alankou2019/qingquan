...
<div class="full_box">
    <div class="full_title">
        <span style="float: right; cursor: pointer;"
            ...
            class="iconfont icon-shuaxin"></i></span> <span class= "name">新建积分考评表</span> <a
            class="go_back" onclick="window.location='{{helper.createUrl(['p':'report/list'])}}';"> <i
            class="iconfont icon-fanhui"></i> <span>返回积分考评表列表</span>
        </a>
    </div>
        ...
        <form action="{{helper.createUrl(['p':'report/save'])}}" method="post" class="form_full" id="dataForm" name="dataForm" enctype="multipart/form-data">
            ...
            <ul class="list_form_full">
                ...
                <li class="posi_lm">
                    ...
                    <div class="right posi_m reportitem" style="width: 80%;">
...
<div style="display: none;">
    <form method="post" action="{{helper.createUrl(['p':'quota/uploadexcel','callbank':'true','point':'true'])}}" enctype="multipart/form-data" id='uploadexccelfrom'>
...
<script>
        ...
        $("body").on("click",".reportfrom", function () {
            ...
            if(val == 1){
                ...
                layer.open({
                    ...
                    shade: 0.8,
                    area: ['50%', '60%'],
                    content:"{{helper.createUrl(['p':'pointreport/simplereporttpl'])}}",
                    btn: ['确定', '取消'],
                    yes: function(index){
    ...
    function afterreporttpl(tplid)
    {
        ...
        $.ajax({
             type: "POST",
             url: "{{helper.createUrl(['p':'pointreport/reporttplitem'])}}",
             data: {id:tplid},
             dataType: "json",
    ...
    function addextra(tplid)
    {
        ...
        $.ajax({
             type: "POST",
             url: "{{helper.createUrl(['p':'pointreport/getextradata'])}}",
             data: {tplid:tplid},
             dataType: "json",
             success: function(res){
                var res = eval(res);
