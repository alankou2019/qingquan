...
<div class="full_box">
    <div class="full_title">
        ...
        <span style="float: right; cursor: pointer;"
            ...
            class="iconfont icon-shuaxin"></i></span> <span class= "name">审核组</span> <a
            class="go_back" onclick="window.location='{{helper.createUrl(['p':'group/index'])}}';"> <i
            class="iconfont icon-fanhui"></i> <span>返回审核组列表</span>
        </a>
    </div>

    <div class="full_cont">
        <!--表单-->
        <form action="{{helper.createUrl(['p':'group/save'])}}" method="post" class="form_full" id="dataForm" name="dataForm" enctype="multipart/form-data">
            <input type="hidden" name="id" value="{{group.id}}">
            <div class="sub_title">审核组权限</div>
            <ul class="list_form_full">
                <li class="posi_lm">
                    <div class="left posi_l must">名称:</div>
                    <div class="right posi_m">
                        <div class="input_clear">
                            <input type="text" name="name"  datatype="*2-11"  value="{{group.name}}"  errormsg="名称2个字符,最多11个字符！"  maxlength="11"/>
                        </div>
                    </div>
                </li>
                <li class="posi_lm">
                    <div class="left posi_l must">描述:</div>
                    <div class="right posi_m">
                        <div class="input_clear">
                            <textarea rows="5" cols="20" name="desc">{{group.desc}}</textarea>
                        </div>
                    </div>
                </li>
                <li class="posi_lm">
