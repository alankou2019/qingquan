...
<script src="/skin/adminhtml/default/libs/laydate/laydate.js"></script>
    ...
    <div class="search_box">
        ...
        <!--搜索按钮-->
        <button class="operate" onclick="exportDatat(1,this)" data-key="{{unique_key}}" data-url="{{helper.createUrl(['p':'stores/export'])}}"><i class="iconfont icon-daochuexcel"></i>导出</button>
        <!--右侧配置-->
        ...
        {% for item in dataList.items %}
        	<tr>
                ...
                <div class="handle">
					...
					<ul class="list">
						<li><a href="{{helper.createUrl(['p':'stores/detail','id':item['id'],'sid':item['sid']])}}">查看详情</a></li>
						<li>
                            <a href="javascript:void(0);"  onclick="exportDatat(1,this)" data-key="{{unique_key}}{{item.id}}" 
                            data-url="{{helper.createUrl(['p':'stores/exportdetail','id':item['id'],'sid':item['sid']])}}">导出详情</a>
                        </li>
					</ul>
				</div>
