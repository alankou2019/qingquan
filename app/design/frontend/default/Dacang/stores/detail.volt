...
<div class="full_box">
	<div class="full_title">
		...
		<a class="go_back" onclick="window.location='{{helper.createUrl(['p':'stores/list'])}}';">
	...
	<div class="full_cont">
		...
		<form action="{{helper.createUrl(['p':'report/save'])}}" method="post" class="form_full" id="dataForm" name="dataForm" enctype="multipart/form-data">
						...
						<table border="1"  width="60%" class="table_box">
						  	...
						  	<tbody class="reporttr">
						  		...
						  		<tr>
							    	...
							    	<td>
							    		<button type='button' onclick='showpoint({{item.id}},{{itemquota.quota_id}},{{itemquota.id}})' class='mybutton'> 查看评分情况  </button>
			...
			<div class="online_btn_box" onclick="window.location='{{helper.createUrl(['p':'stores/list'])}}';">
...
<script>
	...
	function showpoint(reportId,quotaId,sid)
	{
		...
		layer.open({
		    ...
		    shadeClose: true,
		  	shade: 0.8,
		  	area:['600px','520px'],
		  	content:"{{helper.createUrl(['p':'stores/showpoint'])}}"+'?reportId='+reportId+'&quotaId='+quotaId+'&sid='+sid,
		  	btn: ['确定', '取消'],
		  	yes: function(index){
		  		layer.close(index);
		  	}
    ...
    function showcomment(reportId,quotaId,sid)
    {
        ...
        layer.open({
            ...
            shade: 0.8,
            area:['800px','520px'],
            content:"{{helper.createUrl(['p':'stores/quota_comment'])}}"+'?report_id='+reportId+'&quota_id='+quotaId+'&sid='+sid,
            btn: ['确定', '取消'],
            yes: function(index){
                layer.close(index);
            }
