...
<script src="/skin/adminhtml/default/libs/layer/layer.js" type="text/javascript" charset="utf-8"></script>
    ...
    <div class="search_box">
        ...
        <select class="screen" name="filter">
            ...
            <option value="app_key">应用key</option>
        </select>
        <!--搜索筛选-->             
        <!--搜索输入框-->
        <input type="text" name="keywords" value="" placeholder="请输入应用名称" class="search"/>
        <!--搜索输入框-->
        <!--搜索按钮-->
        ...
        <!--搜索按钮-->
        <button class="operate" onclick="window.location='{{helper.createUrl(['p':'app/new'])}}';"><i class="iconfont icon-tianjia"></i>添加应用</button>
        <!--右侧配置-->
...
{% endif %}
    <table class="table_box">
        ...
        <tr class="table_head">
            ...
            <th><span>应用Key</span></th>
            <th><span>应用描述</span></th>
        ...
        {% for item in dataList.items %}
        	<tr>
                ...
                <td class="operate">
                    <button class="btn" onclick="window.location='{{helper.createUrl(['p':'app/edit','id':item.id])}}';"><i class="iconfont icon-caozuo"></i>编辑</button>
                    <button class="delete" onclick="listTable.remove({{item.id}},'您确认要删除吗?');"><i class="iconfont icon-shanchu"></i>删除</button>
                </td>
                <td class="name">
                    <span class="txt">{{item.app_name}}</span>
                </td>
                   <td class="name">
                    <span class="txt">{{item.app_key}}</span>
                </td>
                                <td class="name">
                    <span class="txt">{{item.app_desc}}</span>
                </td>
...
{% if full_page %}
    ...
    <script>
	 ...
	 function searchData()
	 {
    	listTable.filter.ap_id = Utils.trim(document.forms['searchForm'].elements['filter'].value);
    	listTable.filter.adv_device = Utils.trim(document.forms['searchForm'].elements['keywords'].value);
