...
<script src="/skin/adminhtml/default/libs/layer/layer.js" type="text/javascript" charset="utf-8"></script>
    ...
    <div class="search_box">
        ...
        <button class="search_btn btn1"  type="button" onclick="searchData();"><i class="iconfont icon-llhomesearch"></i>搜索</button>
         ...
         <button class="operate" onclick="window.location='{{helper.createUrl(['p':'area/new'])}}';"><i class="iconfont icon-tianjia"></i>添加地区</button>
        <!--右侧配置-->
...
{% endif %}
    <table class="table_box">
        ...
        <tr class="table_head">
            ...
            <th><span>操作</span></th>
            <th><span>地区</span></th>
            <th><span>所在层级</span></th>
            <th><span>所在大区</span></th>
            <th><span>上级地区</span></th>
            <th><span>排序</span></th>
        </tr>
        <!--head部分-->
        <!--列表部分-->
        	...
        	<tr>
                ...
                <td class="operate">
                		<div class="handle">
							...
							<ul class="list">
								<li><a href="{{helper.createUrl(['p':'area/edit','id':item.id])}}">编辑地区</a></li>
                                <li><a href="{{helper.createUrl(['p':'area/new','parent_id':item.id])}}">新增下级</a></li>
								<li><a href="{{helper.createUrl(['p':'area/index','parent_id':item.id])}}">查看下级</a></li>
                                <li><a href="{{helper.createUrl(['p':'area/back','child_id':item.id])}}">返回上级</a></li>
							</ul>
						</div>
                ...
                <td class="name">
                    <span class="txt">{{item.deep}}</span>
                </td>
                  <td class="name">
                  <span class="txt">{{item.region}}</span>
                </td>
                <td class="name">
                    <span class="txt">{{item.parent_id}}</span>
                </td>
                <td class="name">
