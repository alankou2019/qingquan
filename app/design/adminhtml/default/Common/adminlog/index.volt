...
<script src="/skin/adminhtml/default/libs/laydate/laydate.js"></script>
    ...
    <div class="head_tab clear">
        <ul>
            <li class="on"><a href="#">操作日志列表</a></li>
        </ul>
    </div>
    <!--头部TAB切换-->
    ...
    <div class="search_box">
        ...
        <select class="screen" name="filter">
            ...
            <option value="u.realname">操作人名字</option>
        </select>
        <!--搜索筛选-->             
        <!--搜索输入框-->
        <input type="text" name="keywords" value="" placeholder="请输入搜索条件" class="search"/>
        <!--搜索输入框-->
        <span class="time_li"><input placeholder="开始日期" class="laydate-icon dateinput time_li" name="start_time"></span>            
      	<span class="time_li"><input placeholder="结束日期" class="laydate-icon dateinput" name="end_time"></span>
        <!--搜索按钮-->
...
{% if full_page %}
    ...
    <div class="table_foot">
        ...
        <div class="all_delete" style="width:140px;">
            <button onclick="clearLog('1')" style="width:140px;">删除1个月前的数据</button>
        </div>
        <div class="all_delete" style="width:140px;">
            <button onclick="clearLog('3')" style="width:140px;">删除3个月前的数据</button>
        </div>
        <div class="page_box ">
    ...
    <script><!--
	 ...
	 function clearLog(m)
	 {
		 ...
		 else
		 {
			 if (confirm('您确认要删除吗?'))
			 {
				var args = "act=remove" + "&month=" + m + listTable.compileFilter();
				$.ajax({
					  type: 'GET',
					  url: listTable.url,
					  data: args,
