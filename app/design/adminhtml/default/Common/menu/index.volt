...
<script src="/skin/adminhtml/default/libs/layer/layer.js" type="text/javascript" charset="utf-8"></script>
    ...
    <div class="search_box">
        ...
        <!--搜索按钮-->
        <button class="operate" onclick="window.location='{{helper.createUrl(['p':'menu/new'])}}';"><i class="iconfont icon-tianjia"></i>添加菜单</button>
        <!--右侧配置-->
    ...
    <table class="table_box">
        ...
        {% for item in dataList.items %}
        	<tr>
                ...
                <td class="operate">
                    <button class="btn" onclick="window.location='{{helper.createUrl(['p':'menu/edit','id':item.id])}}';"><i class="iconfont icon-caozuo"></i>编辑</button>
                    <button class="delete" onclick="listTable.remove({{item.id}},'您确认要删除吗?');"><i class="iconfont icon-shanchu"></i>删除</button>
                </td>
                <td class="name">
                    <span class="txt">{{item.delimiter}}{{item.name}}</span>
                </td>
                <td class="name">
               		<span class="txt">
                    {% if item.new_window %}
                ...
                <td class="name">
                    <span class="txt">{% if item.link %}<a href="{{item.link}}" target="_blank">{{item.link}}</a>{%endif%}</span>
                </td>
            </tr>
        {% endfor %}
        <!--列表部分-->
