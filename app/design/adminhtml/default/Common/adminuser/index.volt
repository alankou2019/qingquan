...
<script src="/skin/adminhtml/default/libs/layer/layer.js" type="text/javascript" charset="utf-8"></script>
    ...
    <div class="search_box">
        ...
        <select class="screen" name="filter">
            ...
            <option value="a.realname">管理员名称</option>
            <option value="a.username">登录名</option>
        </select>
        <!--搜索筛选-->             
        <!--搜索输入框-->
        ...
        <!--搜索按钮-->
        <button class="operate" onclick="window.location='{{helper.createUrl(['p':'adminuser/new'])}}';"><i class="iconfont icon-tianjia"></i>添加管理员</button>
        <!--右侧配置-->
    ...
    <table class="table_box">
        ...
        {% for item in dataList.items %}
        	<tr>
                ...
                <td class="operate">
                    <button class="btn" onclick="window.location='{{helper.createUrl(['p':'adminuser/edit','id':item.user_id])}}';"><i class="iconfont icon-caozuo"></i>编辑</button>
                ...
                <td class="name">
                    <span class="txt">{{item.username}}</span>
                                ...
                                <td class="name">
                    <span class="txt">{{item.realname}}</span>
                </td>
                  <td class="name">
                  	{%if item.role_id>0%}
                    <span class="txt">{{item.role_name}}</span>
                    {%else%}
                     <span class="txt">超级管理员</span>
                    {%endif%}
                </td>
                ...
                <td class="name">
                    <span class="txt">{{item.lognum}}</span>
                </td>
