...
<script src="/skin/adminhtml/default/libs/layer/layer.js" type="text/javascript" charset="utf-8"></script>
    ...
    <div class="search_box">
        ...
        <!--搜索筛选-->             
        <!--搜索输入框-->
        <input type="text" name="keywords" value="" placeholder="请输入文章标题" class="search"/>
        <!--搜索输入框-->
        <!--搜索按钮-->
        <button class="search_btn btn1"  type="button" onclick="searchData();"><i class="iconfont icon-llhomesearch"></i>搜索</button>
        </form>
        <!--搜索按钮-->
        <button class="operate" onclick="window.location='{{helper.createUrl(['p':'page/new'])}}';"><i class="iconfont icon-tianjia"></i>添加单页</button>
        <!--右侧配置-->
    ...
    <table class="table_box">
        ...
        {% for item in dataList.items %}
        	<tr>
                ...
                <td class="operate">
                    <button class="btn" onclick="window.location='{{helper.createUrl(['p':'page/edit','id':item.id])}}';"><i class="iconfont icon-caozuo"></i>编辑</button>
                    <button class="delete" onclick="listTable.remove({{item.id}},'您确认要删除吗?');"><i class="iconfont icon-shanchu"></i>删除</button>
                </td>
                <td class="name">
                    <span class="txt">{{helper.substr(item.name,0,30)}}</span>
                </td>
                <td class="name">
                    <span class="txt">{{item.cat_name}}({{item.keycode}})</span>
                </td>
                <td class="name">
