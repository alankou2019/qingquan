...
<script src="/skin/adminhtml/default/libs/layer/layer.js" type="text/javascript" charset="utf-8"></script>
    ...
    <div class="head_tab clear">
        <ul>
            <li class="on"><a href="#">单页分类列表</a></li>
        </ul>
    </div>
    <!--头部TAB切换-->
    ...
    <div class="search_box">
        ...
        <!--搜索按钮-->
        <button class="operate" onclick="window.location='{{helper.createUrl(['p':'pagecategory/new'])}}';"><i class="iconfont icon-tianjia"></i>添加分类</button>
        <!--右侧配置-->
    ...
    <table class="table_box">
        ...
        {% for item in dataList.items %}
        	<tr>
                ...
                <td class="operate">
                    <button class="btn" onclick="window.location='{{helper.createUrl(['p':'pagecategory/edit','id':item.id])}}';"><i class="iconfont icon-caozuo"></i>编辑</button>
                ...
                <td class="name">
                    <span class="txt">{{item.keycode}}</span>
                </td>
                <td class="name">
                    <span class="txt">
                    {{item.sort}}
