...
<script src="/skin/adminhtml/default/libs/layer/layer.js" type="text/javascript" charset="utf-8"></script>
    ...
    <div class="head_tab clear">
        <ul>
            <li class="on"><a href="#">支付方式列表</a></li>
        </ul>
    </div>
    <!--头部TAB切换-->
    ...
    <div class="search_box">
        ...
        <!--<button class="operate" style="width:100px;" onclick="window.location='{{helper.createUrl(['p':'payment/new'])}}';"><i class="iconfont icon-tianjia"></i>添加支付方式</button>-->
        <!--右侧配置-->
...
{% endif %}
    <table class="table_box">
        ...
        <tr class="table_head">
            ...
            <th class="image"><span>图标</span></th> 
            <th class="state"><span>状态</span></th>
            <th><span>描述</span></th>
        ...
        {% for item in dataList.items %}
        	<tr>
                ...
                <td class="operate">
                    <button class="btn" onclick="window.location='{{helper.createUrl(['p':'payment/edit','id':item.id])}}';"><i class="iconfont icon-caozuo"></i>编辑</button>
                    <button class="delete" onclick="listTable.remove({{item.id}},'您确认要删除吗?');"><i class="iconfont icon-shanchu"></i>关闭</button>
                </td>
                                <td class="name">
                    <span class="txt">{{helper.substr(item.name,0,30,'utf-8')}}</span>
                </td>
                <td class="image">
                	{% if item.logo %}
                    <img src="{{item.logo}}" width="20" height="20" data-src="{{item.logo}}"/>
                    {% endif %}
                </td>    
                <td class="state">
                    ...
                    {% if item.status %}
   						<span class="yes"> 
                   	  	  <i class="iconfont icon-zhengque"></i>启用
                        </span>
                  	{% else %}
                    	<span class="no"> 
                   	  	  <i class="iconfont icon-jinzhi"></i>禁用
                        </span>
                    {% endif %}
