...
<script src="/skin/adminhtml/default/libs/layer/layer.js" type="text/javascript" charset="utf-8"></script>
    ...
    <div class="search_box">
        ...
        <!--搜索按钮-->
        <button class="operate" onclick="window.location='{{helper.createUrl(['p':'quotatpl/edit'])}}';"><i class="iconfont icon-tianjia"></i>添加指标</button>
    ...
    <table class="table_box" style='width: 70%;'>
        ...
        {% for item in dataList.items %}
        	...
        	<tr>
                ...
                <td class="operate">
                    <button class="btn" onclick="window.location='{{helper.createUrl(['p':'quotatpl/edit','id':item['id']])}}';"><i class="iconfont icon-caozuo"></i>编辑</button>
                ...
                <td class="name">
                    <span class="txt" title="{{item['point_desc']}}">{{helper.substr(item['remark'],0,50)}}</span>
                </td>
            </tr>
        {% endfor %}
        <!--列表部分-->
...
{% if full_page %}
   ...
   <div style="display: none;">
   		<form method="post" action="{{helper.createUrl(['p':'quotatpl/uploadexcel'])}}" enctype="multipart/form-data" id='uploadexccelfrom'>
    ...
    <script>
	 ...
	 {
		 window.location.href = "{{helper.createUrl(['p':'quotatpl/exportexceltpl'])}}";
	 }
