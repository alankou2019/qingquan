...
<script src="/skin/adminhtml/default/libs/layer/layer.js" type="text/javascript" charset="utf-8"></script>
    ...
    <div class="search_box">
        ...
        <select class="screen" name="filter">
            ...
            <option value="a.name">广告名称</option>
            <option value="p.key_code">广告位标识</option>
        </select>
        <!--搜索筛选-->             
        <!--搜索输入框-->
        ...
        <!--搜索按钮-->
        <button class="operate" onclick="window.location='{{helper.createUrl(['p':'advert/new'])}}';"><i class="iconfont icon-tianjia"></i>添加广告</button>
        <!--右侧配置-->
...
{% endif %}
    <table class="table_box">
        ...
        <tr class="table_head">
            ...
            <th><span>广告名称</span></th>  
            <th class="image"><span>图片</span></th>
            <th><span>广告位</span></th>
            <th><span>排序</span></th>  
        ...
        {% for item in dataList.items %}
        	<tr>
                ...
                <td class="operate">
                    <button class="btn" onclick="window.location='{{helper.createUrl(['p':'advert/edit','id':item.id])}}';"><i class="iconfont icon-caozuo"></i>编辑</button>
				...
				<td class="image"><img src="/skin/adminhtml/default/images/1_05.png"  data-src="{{item.content}}"/></td>
                <td class="name">
                    <span class="txt">{{item.position_name}}({{item.key_code}})</span>
                </td>
                                <td class="name">
                    <span class="txt">{{item.sort}}</span>
                </td>
                <td class="name">
                    <span class="txt">{{helper.formatDateTime(item.start_time)}}</span>
                </td>
                <td class="name">
                    <span class="txt">{{helper.formatDateTime(item.end_time)}}</span>
                </td>
                <td class="name">
