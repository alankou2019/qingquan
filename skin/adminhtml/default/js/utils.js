...
{
	var layer = layer == undefined?'':layer;
...
Utils.open = function(url,title,height,width){
	var w = width==undefined?'800px':width;
	var h = height==undefined?'80%':height;
	return layer.open({
		  type: 2,
...
Utils.closeIndex = function(index)
{
	layer.close(index);
}
 ...
 */
Utils.validate = function (validateForm,btn,callback){
     var config = {
                ignoreHidden : true,
                ...
                tiptype : function(msg,o,cssctl){
                    ...
                    if(status == 3){
						...
						$(dom).parent().parent().addClass('wrong');
						if($em.length < 1){
							 $(dom).parent().append('<small style="" class="help-block prompt_box"><i class="fa fa-times-circle-o"></i>'+msg+'</small>');
						}else{
